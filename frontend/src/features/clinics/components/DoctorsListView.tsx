        ...
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 space-y-4">
            <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3">
              ...
              <Button type="submit" className="h-12 px-6 rounded-xl bg-primary text-base font-semibold text-white hover:bg-primary/95 shadow-md">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1 overflow-x-auto pb-1 no-scrollbar">
                ...
                {specialties.map((spec) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-1.5 text-sm font-medium transition-all duration-200 border whitespace-nowrap ${
                      specialtyId === spec.id
                        ? "bg-primary text-white border-primary shadow-sm"
                        : "bg-white text-slate-600 border-slate-200 hover:border-slate-300 hover:bg-slate-50"
                    }`}
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {isDoctorsLoading ? (
              ...
              doctors.map((doctor) => (
                <Card key={doctor.id} className="overflow-hidden border-none shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl group h-full flex flex-col p-0 gap-0">
                  ...
                  <CardHeader className="p-4 pt-5 pb-2 flex-1">
                    <Link href={`/doctor/${doctor.id}`}>
                      <CardTitle className="text-base sm:text-lg font-bold group-hover:text-primary transition-colors line-clamp-1">{doctor.name}</CardTitle>
                    </Link>
                    {doctor.clinic?.name && (
                      <p className="text-xs text-slate-400 font-medium line-clamp-1 mt-0.5">{doctor.clinic.name}</p>
                    )}
                  ...
                  <CardContent className="p-4 pt-0 flex-none flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Link href={`/doctor/${doctor.id}`}>
                        <Button variant="outline" className="h-10 w-full text-xs font-semibold bg-transparent">
                      ...
                      <Link href={`${ROUTES.BOOKING}?doctorId=${doctor.id}&clinicId=${doctor.clinic?.id || ""}&specialtyId=${doctor.specialty?.id || ""}`}>
                        <Button className="h-10 w-full text-xs font-semibold bg-primary hover:bg-primary/90 text-white shadow-sm transition-all active:scale-95">
              ...
              <Card className="col-span-full py-16 flex flex-col items-center justify-center text-center bg-white border border-slate-100 rounded-2xl">
